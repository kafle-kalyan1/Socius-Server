...
class RecommendedFriends(APIView):
    ...
    def get(self, request):
        try:
            ...
            # Get friend requests related to the current user
            ...
            # Exclude the current user from the friends set
            ...
            return Response({'status_code': 200, 'data': recommended_friends_list}, status=status.HTTP_200_OK)
        except Exception as e:
            print(e)
            return Response({'message': "Something went wrong on the server", 'status_code': 500},
    ...
    def get(self, request, username):
        try:
            ...
            if profile:
                ...
                return Response({'status_code': 200, 'data': final_data, 'posts': None},
                                status=status.HTTP_200_OK)
            else:
                return Response({'message': "User profile not found", 'status_code': 404},
...
class FriendRequestBase(APIView):
    ...
    def post(self, request):
        try:
            ...
            friend_profile = UserProfile.objects.get(user__username=friend_username)

            if user_profile != friend_profile:
                return self.handle_friend_request(user_profile, friend_profile)
            else:
                return Response({'status_code': 400, 'message': f'You cannot {self.action} yourself'},
...
class SendFriendRequest(FriendRequestBase):
    ...
    def handle_friend_request(self, user_profile, friend_profile):
        friend_request = FriendRequest.objects.create(sender=user_profile.user, receiver=friend_profile.user, status='pending')
        friend_request.save()
        return Response({'status_code': 200, 'message': 'Friend request sent'}, status=status.HTTP_200_OK)

class AcceptFriendRequest(FriendRequestBase):
    action = 'accept friend request'

    def handle_friend_request(self, user_profile, friend_profile):
        friend_request = FriendRequest.objects.get(sender=friend_profile.user, receiver=user_profile.user, status='pending')
        friend_request.status = 'accepted'
        friend_request.save()
        # Friendship.objects.create(user1=user_profile, user2=friend_profile, status='accepted')
        Friendship.objects.create(user1=user_profile.user, user2=friend_profile.user, status='accepted')
        return Response({'status_code': 200, 'message': 'Friend request accepted'}, status=status.HTTP_200_OK)
...
class RejectFriendRequest(FriendRequestBase):
    ...
    def handle_friend_request(self, user_profile, friend_profile):
        ...
        friend_request.save()
        return Response({'status_code': 200, 'message': 'Friend request rejected'}, status=status.HTTP_200_OK)

class CancelFriendRequest(FriendRequestBase):
    action = 'cancel friend request'
    def handle_friend_request(self, user_profile, friend_profile):
        print(user_profile, friend_profile)
        friend_request = FriendRequest.objects.get(sender=user_profile.user, receiver=friend_profile.user, status='pending')
        print(friend_request.query)
        friend_request.delete()
        return Response({'status_code': 200, 'message': 'Friend request cancelled'}, status=status.HTTP_200_OK)
...
class RemoveFriend(FriendRequestBase):
    ...
    def handle_friend_request(self, user_profile, friend_profile):
        ...
        friendship.delete()
        return Response({'status_code': 200, 'message': 'Friend removed'}, status=status.HTTP_200_OK)
